...
class PatchTSTCat(nn.Module):
    def __init__(self, configs, max_seq_len:Optional[int]=1024, dimension_keys:Optional[int]=None, dimension_values:Optional[int]=None, norm:str='BatchNorm', attn_dropout:float=0., 
        ...
        context_window = configs['context_length']
    ...
    def forward(self, x, cat_features=None):           # x: [Batch, Input length, Channel]
        if self.cat_dims and cat_features is not None:
            # Embed categorical features
            cat_embedded = self.categorical_embedding(cat_features)
